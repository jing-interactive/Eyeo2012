...
Star::Star( vec3 pos, float appMag, float absMag, float color, std::string name, std::string spectrum, const Font &fontS, const Font &fontM )
	: mPos( pos ), mApparentMag( appMag ), mAbsoluteMag( absMag ), mColor( color ), mName( name )
{
	...
	if( mName.length() > 1 && appMag < 6.0f ){
		...
		mNameTex = gl::Texture::create( layout.render( true, false ) );
...
void Star::drawName( const vec2 &mousePos, float power, float alpha )
{
	if( mDistToCam > 0.0f && mNameTex ){
		float per = constrain( 1.0f - mDistToCam * 0.0000375f, 0.0f, 1.0f );
		per *= per * per;
		
		vec2 dirToMouse	= mScreenPos - mousePos;
		...
		if( per > 0.05f ){
			...
			gl::draw( mNameTex, mScreenPos + vec2( mScreenRadius + 35.0f, -28.0f ) );
			mNameTex.disable();
				
			gl::color( ColorA( power, power, power, per * 0.4f * alpha ) );
			vec2 p1 = mScreenPos;
			vec2 p2 = p1 + vec2( mScreenRadius + 35.0f, -13.0f );
			vec2 p3 = mScreenPos + vec2( mScreenRadius + 35.0f + mNameTex.getWidth(), -13.0f );
			gl::drawLine( p1, p2 );
			gl::drawLine( p2, p3 );
		}
	}
