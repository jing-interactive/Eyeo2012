...
//		mPredators.push_back( Predator( vec3() ) );
//	}
}
...
//	while( it.line() ){
//		while( it.pixel() ){
//			mPredators[index].update( vec3( it.r(), it.g(), it.b() ) );
//			index ++;
//		}
//	}
...
//			mPredators[index].update( vec3( pixel[0], pixel[1], pixel[2] ) );
//			index ++;
//		}
//	}
...
void Controller::drawLanternGlows( const vec3 &right, const vec3 &up )
{
	for( std::vector<Lantern>::iterator it = mLanterns.begin(); it != mLanterns.end(); ++it ){
		float radius = it->mRadius * 10.0f;// * it->mVisiblePer * 10.0f;
		gl::color( Color( it->mColor ) );
		gl::drawBillboard( it->mPos, vec2( radius, radius ), 0.0f, right, up );
		gl::color( Color::white() );
		gl::drawBillboard( it->mPos, vec2( radius, radius ) * 0.35f, 0.0f, right, up );
	}
}
...
void Controller::addLantern( const vec3 &pos )
{
...
void Controller::addGlows( Lantern *lantern, int amt )
{
	for( int i=0; i<amt; i++ ){
		...
		vec3 pos		= lantern->mPos + dir * ( lantern->mRadius - radius );
		dir.xz() *= -0.25f;
		vec3 vel		= dir * Rand::randFloat( 0.3f, 0.5f );
		...
		if( Rand::randFloat() < 0.01f )
			mGlows.push_back( Glow( pos, vec3( vel.x, 1.5f, vel.z ), radius * 0.5f, col, lifespan * 0.5f ) );
	}
}
...
void Controller::addNebulas( Lantern *lantern, int amt )
{
	for( int i=0; i<amt; i++ ){
		...
		vec3 pos			= lantern->mPos + dir * ( lantern->mRadius - radius );
		dir.xz() *= -0.25f;
		vec3 vel			= dir * Rand::randFloat( 0.05f, 0.1f );
		...
		if( Rand::randFloat() < 0.01f )
			mNebulas.push_back( Nebula( pos, vec3( vel.x, 1.5f, vel.z ), radius * 0.5f, col, lifespan * 0.5f ) );
	}
}
